...
class PokemonRedAgent:
    ...
    This is the proven working version integrated with RL training capabilities
    """
    ...
    def __init__(self, rom_path, show_window=True):
        ...
        """
        # Store for RL integration
        self.rom_path = rom_path
        self.show_window = show_window

        # Initialize PyBoy with appropriate window type
        ...
        # Button mappings for PyBoy 2.x (uses string identifiers)
        self.buttons = {
            'A': 'a',
            'B': 'b',
            'SELECT': 'select',
    ...
    def press_button(self, button, hold_frames=10, release_frames=5):
        """
        ...
        try:
            ...
            old_button_map = {'A': 0, 'B': 1, 'SELECT': 2, 'START': 3,
                              'RIGHT': 4, 'LEFT': 5, 'UP': 6, 'DOWN': 7}

            if button in old_button_map:
                self.pyboy.send_input(old_button_map[button])
                for _ in range(hold_frames):
                    self.pyboy.tick()
                self.pyboy.send_input(old_button_map[button], False)
                for _ in range(release_frames):
                    self.pyboy.tick()
    ...
    def get_tilemap_background(self):
        ...
        return np.array(self.pyboy.tilemap_background)

    def read_memory_value(self, address, is_16bit=False):
    ...
    def get_current_screen_type(self):
        ...
        try:
            ...
            try:
                tilemap = self.get_tilemap_background()
    ...
    def wait_for_screen_change(self, current_screen, timeout_seconds=10):
        """Wait for the screen to change from current state"""
        timeout_frames = timeout_seconds * 60

        for frame in range(timeout_frames):
            new_screen = self.get_current_screen_type()
            if new_screen != current_screen and new_screen != "unknown":
                return new_screen
            ...
            if frame % 60 == 0:
                ...
                if seconds_elapsed > 0:  # Don't log at 0 seconds
                    print(f"  Waiting for screen change... {seconds_elapsed}/{timeout_seconds}s")

        return self.get_current_screen_type()

    def smart_button_press(self, button, screen_type):
        """Press button and wait for screen to change, trying multiple methods"""
        current_screen = screen_type

        # Try different button press methods (PyBoy 2.x compatibility)
        methods = [
            ("button_press/release", self._method_button_press),
            ("send_input strings", self._method_send_input_string),
            ("send_input numbers", self._method_send_input_number)
        ]
        ...
        for method_name, method_func in methods:
            try:
                ...
                # Wait for screen change with 3 second timeout
                new_screen = self.wait_for_screen_change(current_screen, timeout_seconds=3)
    ...
    def _method_button_press(self, button):
        """Button press using PyBoy 2.x button_press/button_release"""
        self.pyboy.button_press(self.buttons[button])
        self.wait_frames(15)  # Hold for 15 frames (~250ms)
        self.pyboy.button_release(self.buttons[button])

    def _method_send_input_string(self, button):
        """Button press using send_input with string identifiers"""
        self.pyboy.send_input(self.buttons[button])
        self.wait_frames(15)
        self.pyboy.send_input(self.buttons[button], False)

    def _method_send_input_number(self, button):
        """Button press using send_input with numeric IDs (PyBoy 1.x style)"""
        old_buttons = {'A': 0, 'B': 1, 'SELECT': 2, 'START': 3,
                       'RIGHT': 4, 'LEFT': 5, 'UP': 6, 'DOWN': 7}
        if button in old_buttons:
            self.pyboy.send_input(old_buttons[button])
            self.wait_frames(15)
            self.pyboy.send_input(old_buttons[button], False)
    ...
    def skip_intro_sequence(self):
        ...
        for attempt in range(max_attempts):
            ...
            if current_screen == "title_screen":
                ...
                new_screen = self.smart_button_press('START', current_screen)
                if new_screen == current_screen:
                    # Try A button if START doesn't work
                    new_screen = self.smart_button_press('A', current_screen)

            elif current_screen == "intro_animation":
                ...
                new_screen = self.smart_button_press('A', current_screen)
                if new_screen == "in_game" or new_screen != current_screen:
                    return True  # Successfully started game
                ...
                for button in ['START', 'A']:
                    self.smart_button_press(button, current_screen)
                    self.wait_frames(60)  # 1 second between attempts

            # Brief pause between navigation attempts
            self.wait_frames(60)

        return False  # Failed to navigate intro sequence

    def skip_professor_oak_intro(self):
        ...
        for i in range(30):
            try:
                ...
                self.wait_frames(5)  # Brief hold
                self.pyboy.button_release('a')
                self.wait_frames(20)  # Wait for text to advance (~333ms)
            except:
                # Fallback to send_input method
                self.pyboy.send_input(0)  # A button = 0
                self.wait_frames(5)
                self.pyboy.send_input(0, False)
                self.wait_frames(20)

    def handle_naming_screen(self, name="RED", is_player=True):
        """Handle Pokemon Red naming screen by accepting default names"""
        target = "player" if is_player else "rival"

        # Wait for naming screen to load (2 seconds)
        self.wait_frames(120)

        # Method 1: Press START to accept default name (most reliable)
        for attempt in range(5):
            self.press_button('START', hold_frames=20, release_frames=60)
            self.wait_frames(60)

        # Method 2: Navigate to END button manually
        # Move cursor to bottom-right where END button typically is located

        # Move all the way down (8 presses covers full screen height)
        for _ in range(8):
            self.press_button('DOWN', hold_frames=10, release_frames=15)

        # Move all the way right (10 presses covers full screen width)
        for _ in range(10):
            self.press_button('RIGHT', hold_frames=10, release_frames=15)

        # Try pressing A on END button
        for attempt in range(3):
            self.press_button('A', hold_frames=20, release_frames=60)
            self.wait_frames(60)

        # Method 3: Try alternative END button positions
        alternative_positions = [
            (5, 9),  # Standard bottom-right
            (4, 9),  # One row up from bottom
            (5, 8),  # One column left from right edge
            (4, 8),  # Diagonal up-left from corner
        ]

        for row_moves, col_moves in alternative_positions:
            # Reset cursor to top-left corner
            for _ in range(10):
                self.press_button('UP', hold_frames=8, release_frames=8)
                self.press_button('LEFT', hold_frames=8, release_frames=8)

            # Move to specific position
            for _ in range(row_moves):
                self.press_button('DOWN', hold_frames=8, release_frames=8)
            for _ in range(col_moves):
                self.press_button('RIGHT', hold_frames=8, release_frames=8)

            # Try confirming at this position
            self.press_button('A', hold_frames=20, release_frames=60)
            self.wait_frames(30)

        # Extra wait to ensure screen transition completes (3 seconds)
        self.wait_frames(180)

    def complete_intro_dialogue(self):
        """Complete remaining intro dialogue after naming sequence"""
        # Continue pressing A to advance through dialogue (20 presses)
        for _ in range(20):
            self.press_button('A', hold_frames=5, release_frames=25)

        # Wait for world/lab to load (2 seconds)
        self.wait_frames(120)

    def take_initial_steps(self):
    ...
    def run_opening_sequence(self, player_name="RED", rival_name="BLUE"):
        """
        Complete the entire Pokemon Red opening sequence automatically
        This is the proven working method from your standalone code
        """
        try:
            ...
            print(f"Handling player naming (accepting default)...")
            self.handle_naming_screen(player_name, is_player=True)

            print(f"Handling rival naming (accepting default)...")
            ...
            print(f"Error during opening sequence: {e}")
            return False
    ...
    def reset_game(self):
        ...
        return self.run_opening_sequence()
    ...
    def cleanup(self):
        ...
        try:
            ...
            self.pyboy.stop()
            print("PyBoy stopped successfully")
        except Exception as e:
            print(f"Error during PyBoy cleanup: {e}")
...
class PokemonRedGymEnv(gym.Env):
    ...
    def _check_done(self) -> bool:
        """Check if episode should end - LENIENT VERSION"""
        # Only end episodes for max steps reached
        if self.episode_steps >= self.max_episode_steps:
            print(f"Episode ended: Max steps reached ({self.max_episode_steps})")
            return True

        # Never end for any other reason during training
        return False
    ...
    def step(self, action: int) -> Tuple[Dict[str, np.ndarray], float, bool, bool, Dict[str, Any]]:
        ...
        if self.episode_steps % 100 == 0:
            position = self.game.get_player_position()
            print(f"Step {self.episode_steps}: Pos{position}, Reward: {reward:.2f}, Total: {self.episode_reward:.1f}")
    ...
    def reset(self, seed: Optional[int] = None, options: Optional[dict] = None) -> Tuple[
        ...
        print(f"Environment reset called (episode steps were: {self.episode_steps})")
        ...
        # Reset game using proven working method
        print("Starting game reset...")
        ...
        print("Reset complete! Starting new episode.")
...
class TrainingCallback(BaseCallback):
    ...
    def _on_rollout_end(self) -> None:
        ...
        if hasattr(self.model, 'ep_info_buffer') and len(self.model.ep_info_buffer) > 0:
            try:
                # Convert to list if needed and get last 10 episodes
                ...
                lengths = [ep['l'] for ep in recent_episodes if 'l' in ep]
                ...
                if self.verbose >= 1:
                    print(f"Warning: Episode buffer access failed: {e}")
...
class EnhancedTrainingCallback(BaseCallback):
    """Enhanced callback with live visualizations"""
        ...
        if len(self.model.ep_info_buffer) > 0:
            ...
            for episode_info in recent_episodes:
                ...
                if reward > self.best_reward:
                    ...
                    if self.verbose >= 1:
                        print(f"New best model! Reward: {reward:.2f}, Maps: {maps}, Badges: {badges}")
...
class PokemonTrainer:
    ...
    def cleanup_save_files(self):
        """Remove save files to ensure clean starts"""
        save_files = [
            self.rom_path + '.ram',
            self.rom_path + '.sav',
            self.rom_path + '.rtc'
        ...
        for save_file in save_files:
            try:
                if os.path.exists(save_file):
                    os.remove(save_file)
                    print(f"Removed save file: {save_file}")
            except Exception as e:
                print(f"Warning: Could not remove {save_file}: {e}")
    ...
    def create_environment(self, headless: bool = True) -> PokemonRedGymEnv:
        ...
        # Skip environment validation to avoid opening sequence loops
        print("Skipping environment validation to avoid opening sequence loops...")
